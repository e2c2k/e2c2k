...
public class LinkedPositionalList<E extends Comparable<E>> implements PositionalList<E> {
  ...
  public int binarySearch(E target){
    ...
    while (left <= right) {
      ...
      int compare = get(middle).compareTo(target);
      numCompare++;
  ...
  public static  void main (String args[]){
    LinkedPositionalList<Integer> test = new LinkedPositionalList<Integer>();
    final int from = 5;
    int to = 100;
    System.out.println("From\tto\tTarget\tn\tlogN\tNlogN\t#Iters\t#Nodes");
    System.out.println("====\t==\t======\t=\t====\t=====\t======\t======");

    for(int j =0;j<1000;j+=100){
      for (int i = 5+j;i<=to;i+=5){
        test.addLast(i);
      }
      test.binarySearch(3);
      int iters = test.numCompare;
      int nodes = test.nodesV;
      int N = test.size();
      int log2N = (int)(Math.log(N) / Math.log(2));
      int nLog2N = N * log2N;
      System.out.println(from+"\t"+to+"\t3\t"+N+"\t"+log2N+"\t"+nLog2N+"\t"+iters+"\t"+nodes);//add math stuff still
      to+=100;
    }

    LinkedPositionalList<Integer> test2 = new LinkedPositionalList<Integer>();

    to = 100;
    System.out.println("\nFrom\tto\tTarget\tn\tlogN\tNlogN\t#Iters\t#Nodes");
    System.out.println("====\t==\t======\t=\t====\t=====\t======\t======");
    ...
    for(int j =0;j<1000;j+=100){
      ...
      test2.binarySearch(to-3);
      int iters = test2.numCompare;
      int nodes = test2.nodesV;
      int N = test2.size();
      int log2N = (int)(Math.log(N) / Math.log(2));
      int nLog2N = N * log2N;
      System.out.println(from+"\t"+to+"\t"+(to-3)+"\t"+N+"\t"+log2N+"\t"+nLog2N+"\t"+iters+"\t"+nodes);//add math stuff still
      to+=100;
    }

    LinkedPositionalList<Integer> test3 = new LinkedPositionalList<Integer>();
    ...
    for(int j =0;j<1000;j+=100){
      ...
      test3.binarySearch((to/2)-1);
      int iters = test3.numCompare;
      int nodes = test3.nodesV;
      int N = test3.size();
      int log2N = (int)(Math.log(N) / Math.log(2));
      int nLog2N = N * log2N;
      System.out.println(from+"\t"+to+"\t"+((to/2)-1)+"\t"+N+"\t"+log2N+"\t"+nLog2N+"\t"+iters+"\t"+nodes);//add math stuff still
      to+=100;
    }
